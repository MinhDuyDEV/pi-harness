...
export const destructiveRules: RuleSet = [
	rule({
		...
		check: (ctx) => {
			...
			const hitsRoot =
				...
				/\brm\s+.*\$HOME/.test(cmd);
			return hitsRoot
				? block("no-catastrophic-rm", "critical", "data-destruction",
	...
	rule({
		...
		check: (ctx) => {
			...
			return dirMatch
				? confirm("warn-bulk-delete-src", "high", "data-destruction",
	...
	rule({
		...
		check: (ctx) => {
			const cmd = ctx.command!;
			return /\bkill\s+-9\b/.test(cmd) || /\bkillall\b/.test(cmd) || /\bpkill\b/.test(cmd)
				? confirm("warn-process-kill", "high", "data-destruction",
	...
	rule({
		...
		check: (ctx) =>
			/\bfind\b.*\s-delete\b/.test(ctx.command!)
				? confirm("warn-find-delete", "medium", "data-destruction",
					"`find -delete` permanently removes matched files. Verify the pattern is correct.")
				: null,
	}),
	rule({
		id: "warn-dd",
		description: "Raw disk operations via dd",
		severity: "high",
		threat: "data-destruction",
		targets: ["bash"],
		check: (ctx) =>
