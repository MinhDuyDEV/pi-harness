...
export default function (pi: ExtensionAPI) {
  ...
  function writeState(state: Record<string, unknown>) {
    try {
      ...
      writeFileSync(
        SYNC_FILE,
        JSON.stringify({ ...state, pid: process.pid, app, bridgeVersion: BRIDGE_VERSION, timestamp: Date.now() }),
        'utf-8'
      )
    } catch {
  ...
  function renderOpenPiMirror(ctx: ExtensionContext, epoch: number) {
    ...
    const hasLivePreviewPayload = state.bridgeVersion === BRIDGE_VERSION && Array.isArray(state.previewMessages)
    const payloadMessages = normalizePreviewMessages(state.previewMessages)
    const historyMessages = sessionFile ? readSessionPreview(sessionFile, HISTORY_PREVIEW_LIMIT) : []
...
class OpenPiMirrorWidget implements Component {
  ...
  render(width: number): string[] {
    ...
    const lines: string[] = []
    const stateLabel = snap.needsOpenPiRestart ? 'restart OpenPi' : snap.stale ? 'stale' : snap.live ? 'live' : 'synced'
    const stateColor = snap.needsOpenPiRestart || snap.stale ? 'warning' : snap.live ? 'success' : 'muted'
    const title = [
    ...
    if (snap.needsOpenPiRestart) {
      lines.push(
        truncateToWidth(
          this.theme.fg('warning', 'OpenPi is using the old bridge. Restart OpenPi to enable live preview.'),
          width
        )
      )
    }
...
function buildLocalPreview(sessionFile: string | null, liveText: string): PreviewMessage[] {
  ...
  return [...messages, { id: 'live-assistant', role: 'assistant', text: trimmedLiveText, live: true }].slice(
...
function normalizePreviewMessages(value: unknown): PreviewMessage[] {
  ...
  return value
    .map((item): PreviewMessage | null => {
      ...
      const id = typeof item.id === 'string' && item.id ? item.id : `${role ?? 'message'}-${text.length}`
