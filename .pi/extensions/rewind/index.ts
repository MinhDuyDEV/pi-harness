...
interface SessionLikeCustomEntry {
  ...
  id: string;
    ...
    // Local mirror of the SDK's SessionEntry union. We keep this in sync
    // with `@earendil-works/pi-coding-agent` rather than importing it
    // directly, so we can stay decoupled from internal type renames.
    // The trailing open member is the forward-compat fallback for new
    // entry types the SDK may add; it shares the same shape as the
    // narrow members so narrowing still works correctly.
...
function findAssistantEntryForTurn(entries: SessionLikeEntry[], message: { timestamp?: number; content?: unknown }): SessionLikeMessageEntry | null {
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    const entry = entries[index];
        if (!isAssistantMessageEntry(entry) || !entry.message) continue;
...
export default function rewindExtension(pi: ExtensionAPI) {
    ...
    const ledger: ParsedSessionLedger = {
      ...
      references: [],
        ...
        for (const rawEntry of ctx.sessionManager.getEntries() as SessionLikeEntry[]) {
          if (rawEntry.type === "custom") {
            const custom = rawEntry as SessionLikeCustomEntry;
            if (custom.customType === "rewind-turn" && isRewindTurnData(custom.data)) {
              applyBindings(ledger.entryToCommit, custom.data.snapshots, custom.data.bindings);
              addReferences(ledger.references, custom.data.snapshots, toTimestamp(rawEntry.timestamp), custom.data);
              continue;
            }
            if (custom.customType === "rewind-op" && isRewindOpData(custom.data)) {
              applyBindings(ledger.entryToCommit, custom.data.snapshots, custom.data.bindings);
              addReferences(ledger.references, custom.data.snapshots, toTimestamp(rawEntry.timestamp), custom.data);
              const currentCommitSha = getCommitFromData(custom.data, "current");
              if (currentCommitSha) ledger.latestCurrentCommitSha = currentCommitSha;
              const undoCommitSha = getCommitFromData(custom.data, "undo");
              if (undoCommitSha) ledger.latestUndoCommitSha = undoCommitSha;
              continue;
            }
            if (custom.customType === "rewind-fork-pending" && isRewindForkPendingData(custom.data)) {
              ledger.latestForkPending = custom.data;
              continue;
            }
          }

          if (rawEntry.type === "label") {
            updateLabelSet(ledger.labeledEntryIds, rawEntry as SessionLikeLabelEntry);
          }
        }
    ...
    try {
      ...
      const lines = content.split("\n").filter(Boolean);
          ...
          for (const line of lines) {
            ...
            if (entry.type === "session") {
              ledger.sessionId = typeof entry.id === "string" ? entry.id : ledger.sessionId;
              if (typeof entry.cwd === "string") ledger.cwd = entry.cwd;
              if (typeof entry.parentSession === "string" || entry.parentSession === null) {
                ledger.parentSession = entry.parentSession ?? undefined;
              }
              continue;
            }
            ...
            if (entry.type === "custom" && customType === "rewind-turn" && isRewindTurnData(data)) {
              const turn = data;
              applyBindings(ledger.entryToCommit, turn.snapshots, turn.bindings);
              addReferences(ledger.references, turn.snapshots, toTimestamp(typeof timestamp === "string" ? timestamp : undefined), turn);
              continue;
            }

            if (entry.type === "custom" && customType === "rewind-op" && isRewindOpData(data)) {
              const op = data;
              applyBindings(ledger.entryToCommit, op.snapshots, op.bindings);
              addReferences(ledger.references, op.snapshots, toTimestamp(typeof timestamp === "string" ? timestamp : undefined), op);
              const currentCommitSha = getCommitFromData(op, "current");
              if (currentCommitSha) ledger.latestCurrentCommitSha = currentCommitSha;
              const undoCommitSha = getCommitFromData(op, "undo");
              if (undoCommitSha) ledger.latestUndoCommitSha = undoCommitSha;
              continue;
            }

            if (entry.type === "custom" && customType === "rewind-fork-pending" && isRewindForkPendingData(data)) {
              const pending = data;
              ledger.latestForkPending = pending;
              continue;
            }

            if (entry.type === "label") {
              updateLabelSet(ledger.labeledEntryIds, entry as unknown as SessionLikeLabelEntry);
            }
          }
  ...
  async function discoverSessionFiles(scanMode: "ancestor-only" | "repo-sessions"): Promise<string[]> {
    ...
    while (ancestorCursor) {
      ...
      const ledger = ancestorCursor === currentSessionFile ? buildCurrentSessionLedgerFromMemory() : await parseSessionLedgerFile(ancestorCursor);
          ancestorCursor = ledger?.parentSession ?? undefined;
        }
    ...
    for (const sessionFile of sessionFiles) {
      if (budgetExceeded()) {
        ...
        return;
              ...
              const ledger = sessionFile === currentSessionFile
                ? buildCurrentSessionLedger(ctx)
                : await parseSessionLedgerFile(sessionFile);
              if (!ledger || !ledger.cwd) continue;
              if (!isInsidePath(ledger.cwd, repoRoot as string)) continue;
              ledgers.push(ledger);
    }
  ...
  pi.on("session_before_fork", async (event, ctx) => {
    ...
    try {
      ...
      console.warn("[rewind-debug] showing picker with options:", options);
      const choice = await ctx.ui.select("Restore Options", options);
      console.warn("[rewind-debug] picker returned:", choice);
      if (!choice) {
  ...
  pi.on("session_before_tree", async (event, ctx) => {
    console.warn("[rewind-debug] session_before_tree fired", {
      isGitRepo,
      hasUI: ctx.hasUI,
    ...
    if (!isGitRepo) {
      console.warn("[rewind-debug] bailing: not a git repo");
      return;
    }

    try {
      if ((globalThis as typeof globalThis & { __boomerangCollapseInProgress?: boolean }).__boomerangCollapseInProgress === true) {
        console.warn("[rewind-debug] bailing: boomerang collapse in progress, snapshotting current state only");
        pendingTreeState = { currentCommitSha: await ensureSnapshotForCurrentWorktree() };
        return;
      }

      if (!ctx.hasUI) {
        console.warn("[rewind-debug] bailing: ctx.hasUI is false");
        return;
      }
      ...
      console.warn("[rewind-debug] target resolution", {
        targetEntryType: targetEntry?.type,
        targetEntryRole: targetEntry && targetEntry.type === "message" ? targetEntry.message?.role : undefined,
        isRestorable: isRestorableTreeEntry(targetEntry),
        targetCommitSha: targetCommitSha?.slice(0, 7),
      });
      const hasUndo = Boolean(activeBranchState.undoCommitSha && (await commitExists(activeBranchState.undoCommitSha)));
