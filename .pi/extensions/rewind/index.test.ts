      ...
      const api = {
        ...
        events: {
          ...
          emit: (eventName: string, data: unknown) => {
            // Tests don't drive the event bus, but the SDK type requires it.
            return eventHandlers.get(eventName)?.(data);
          },
        },
        // The runtime passes the rest of the API via `bindExtensions`;
        // the test harness only needs the methods exercised above.
