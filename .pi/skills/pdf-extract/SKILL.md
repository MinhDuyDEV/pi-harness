...
name: pdf-extract
description: >-
  Chooses the right PDF extraction approach by document type — pdfplumber/camelot for tables, OCR for
  scans, vision models for complex layouts — with a sample-first workflow. User-invoked: load via
  /skill:pdf-extract when extracting text, tables, images, or metadata from PDFs, or batch-processing
  PDFs for RAG.
metadata:
  version: 1.0.0
  tags:
...
<EXTREMELY-IMPORTANT>
- **Match the library to the PDF.** Simple text → `pdf-parse` or `pdfjs`. Structured data → `pdfplumber` (Python) or `camelot`. Complex layouts → vision model.
...
| Mixed (text + tables + images) | Vision model |
...
Wrong library for the PDF type (`pdftotext` on a scanned PDF extracts nothing; `pdf-parse` jumbles multi-column text); OCR on a text-based PDF (slower, less accurate); no sample test before the full run ("worked on 1 page" ≠ works on 500); no verification of output (extraction lies silently — a missing column, a merged cell); tables flattened to text (loses 2D structure); batch processing without error handling (one bad PDF kills the run); "the library says it supports tables" (test it on YOUR tables); "I'll fix the output in post" (extraction should be correct).
