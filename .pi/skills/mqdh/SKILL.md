...
After setting up the environment variable and loading this skill with `/skill:mqdh`, the tools listed below are available directly. Invoke them like:

```
fetch_meta_quest_doc({"query": "hand tracking setup"})
```
...
```
fetch_meta_quest_doc({"query": "passthrough API Unity"})
```
...
fetch_meta_quest_doc({"query": "hand tracking gesture detection"})

# Search for passthrough setup
fetch_meta_quest_doc({"query": "mixed reality passthrough Unity"})
```
...
# Install a build
install_apk({"apk_path": "/builds/myapp.apk", "device_id": "1234567890"})
```
