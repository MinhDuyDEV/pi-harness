...
export default function (pi: ExtensionAPI) {
  ...
  /** Max retry attempts for failed agent tasks. */
   ...
   *  Backward compatible: accepts both RpcReply envelopes and legacy bare payloads.
   */
  function rpcCall<T>(channel: string, params: Record<string, unknown>, timeoutMs: number): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const unsub = pi.events.on(`${channel}:reply:${requestId}`, (raw: unknown) => {
        ...
        // Preferred envelope form
        if (typeof maybe?.success === "boolean") {
          if (maybe.success) resolve(maybe.data as T);
          else reject(new Error(maybe.error ?? `${channel} failed`));
          return;
        }

        // Legacy compatibility: bare payload means success
        resolve(raw as T);
      });
   ...
   *  Ignore malformed replies (e.g. `{}` from legacy handlers) and wait for a valid envelope. */
  function checkSubagentsVersion() {
    ...
    const unsub = pi.events.on(channel, (raw: unknown) => {
      // Ignore malformed replies and keep listening until timeout/valid response
  ...
  pi.events.on("subagents:ready", () => {
    // Debounced by versionCheckInFlight
    checkSubagentsVersion();
  });

  /** Build a prompt for a task being executed by a subagent.
   *  Injects completed dependency results so cascaded agents have full context. */
  function buildTaskPrompt(
    ...
    // Inject results from completed prerequisite tasks (cascade data flow)
    if (task.blockedBy.length > 0) {
      const depResults: string[] = [];
      for (const depId of task.blockedBy) {
        const dep = store.get(depId);
        if (dep?.metadata?.result) {
          // Truncate very long results to avoid blowing context budget
  ...
  // Success → mark task completed, validate episode, cascade if enabled
  pi.events.on("subagents:completed", async (data) => {
    const { id, result, episode } = data as { id: string; result?: string; episode?: { status?: string; blockers?: string[] } };
    ...
    if ((cfg.autoCascade ?? false) && cascadeConfig && latestCtx) {
      // Episode validation: if the completed task reported partial/blocked/failure,
      // don't auto-cascade dependents — the results may be unreliable.
      const episodeStatus = episode?.status;
      const shouldCascade = !episodeStatus || episodeStatus === "success";

      if (!shouldCascade) {
        debug(`cascade:blocked task=#${task.id} — episode status="${episodeStatus}", blockers=${episode?.blockers?.join("; ") ?? "none"}`);
        // Don't cascade, but still mark this task as completed (it did finish)
      } else {
  ...
  // Failure → retry with feedback (up to MAX_TASK_RETRIES), then revert to pending
  ...
  pi.events.on("subagents:failed", async (data) => {
    ...
    if (status === "stopped") {
      ...
      autoClear.trackCompletion(task.id, currentTurn);
    } else if ((cfg.autoCascade ?? false) && cascadeConfig && latestCtx) {
      // Retry-with-feedback: inject failure context into the next attempt
      const retryCount = (task.metadata?.retryCount ?? 0) + 1;

      if (retryCount <= MAX_TASK_RETRIES && task.metadata?.agentType) {
        debug(`retry:attempt task=#${task.id} retry=${retryCount}/${MAX_TASK_RETRIES} error=${error}`);

        const feedbackBlock = [
          `\n\n## ⚠ Previous Attempt Failed (attempt ${retryCount}/${MAX_TASK_RETRIES + 1})`,
          `**Error:** ${error || "Unknown error"}`,
          result ? `**Partial output:**\n${result.slice(0, 2000)}` : "",
          `\nLearn from this failure. Try a different approach if the same strategy failed.`,
        ].filter(Boolean).join("\n");

        const retryPrompt = buildTaskPrompt(task, (cascadeConfig.additionalContext || "") + feedbackBlock);

        try {
          const agentId = await spawnSubagent(task.metadata.agentType, retryPrompt, {
            description: `${task.subject} (retry ${retryCount})`,
            isBackground: true,
            maxTurns: cascadeConfig.maxTurns,
            ...(cascadeConfig.model ? { model: cascadeConfig.model } : {}),
          });
          ...
          store.update(task.id, {
            status: "in_progress",
            owner: agentId,
            metadata: { ...task.metadata, agentId, retryCount, lastError: error || status },
          });
          widget.setActiveTask(task.id);
          widget.update();
          return; // Don't fall through to pending revert
        } catch (spawnErr: any) {
          debug(`retry:spawn-failed task=#${task.id}`, spawnErr);
          // Fall through to pending revert
        }
      } else {
        debug(`retry:exhausted task=#${task.id} retries=${retryCount}`);
      }

      // Max retries exhausted or spawn failed — revert to pending
      store.update(task.id, {
        status: "pending",
        metadata: { ...task.metadata, lastError: error || status, retryCount: retryCount },
      });
      autoClear.resetBatchCountdown();
    } else {
      // No cascade config — simple revert (original behavior)
