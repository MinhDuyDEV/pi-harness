...
import { TaskWidget } from "./ui/task-widget.js";
// ---- Debug ----
...
export default function (pi) {
    ...
    function resolveStorePath(sessionId) {
        ...
        if (taskScope === "session")
            return undefined; // no session ID yet, start in-memory
            ...
            const unsub = pi.events.on(`${channel}:reply:${requestId}`, (raw) => {
                ...
                if (typeof maybe?.success === "boolean") {
                    if (maybe.success)
                        ...
                        reject(new Error(maybe.error ?? `${channel} failed`));
                    return;
                }
    ...
    function spawnSubagent(type, prompt, options) {
        ...
        return rpcCall("subagents:rpc:spawn", { type, prompt, options }, 30_000)
    ...
    function stopSubagent(agentId) {
        return rpcCall("subagents:rpc:stop", { agentId }, 10_000).catch(() => { });
    }
    ...
    function checkSubagentsVersion() {
        ...
        const unsub = pi.events.on(channel, (raw) => {
    ...
    pi.events.on("subagents:completed", async (data) => {
        ...
        if ((cfg.autoCascade ?? false) && cascadeConfig && latestCtx) {
            // Episode-aware cascade gating: don't cascade if episode reports failure/partial/blocked
            const episodeStatus = episode?.status;
            const shouldCascade = !episodeStatus || episodeStatus === "success";
            if (!shouldCascade) {
                debug(`cascade:blocked task=#${task.id} — episode status="${episodeStatus}"`);
            }
            else {
                const unblocked = store.list().filter(t => t.status === "pending" &&
    ...
    pi.events.on("subagents:failed", async (data) => {
        ...
        if (status === "stopped") {
            ...
            // Retry-with-feedback: inject error + partial output for the next attempt
            const retryCount = (task.metadata?.retryCount || 0) + 1;
            if (retryCount <= MAX_TASK_RETRIES && task.metadata?.agentType && cascadeConfig && latestCtx) {
                debug(`retry task=#${task.id} attempt=${retryCount}/${MAX_TASK_RETRIES}`);
                const feedbackBlock = [
                ...
                store.update(task.id, { status: "in_progress", metadata: { ...task.metadata, retryCount } });
                const prompt = buildTaskPrompt(task, feedbackBlock);
                try {
                    ...
                    store.update(task.id, { owner: agentId, metadata: { ...task.metadata, agentId, retryCount } });
                    widget.setActiveTask(task.id);
                    widget.update();
                    return;
                }
                catch (retryErr) {
                    debug(`retry:spawn:error task=#${task.id}`, retryErr);
                }
            }
...
- Works with all task types: background shells, async agents, and remote sessions`,
        ...
        async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
            ...
            if (block && processOutput.status === "running") {
                ...
                if (result) {
                    return textResult(`Task #${task_id} (${result.status})${result.exitCode !== undefined ? ` exit code: ${result.exitCode}` : ""}\n\n${result.output}`);
                }
            }
            return textResult(`Task #${task_id} (${processOutput.status})${processOutput.exitCode !== undefined ? ` exit code: ${processOutput.exitCode}` : ""}\n\n${processOutput.output}`);
        },
    });
    // ──────────────────────────────────────────────────
    // Tool 6: TaskStop
