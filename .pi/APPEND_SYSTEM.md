...
**Completion gate:** Non-trivial code changed this session → `task(reviewer)` with paths touched, or `REVIEW_SKIPPED: <reason>` before claiming done. Tiny mechanical edits may self-review with cited verification. `harness-*` → `harness` tool only.
